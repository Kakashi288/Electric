...
function handleSubMenuTransition() {
 ...
 // Check if the viewport is ≤1200px
...
function handleOurServicesClick(event) {
 // Prevent the default behavior if the viewport width is ≤ 1200px
 event.preventDefault();
 // Toggling is handled by handleDropdownClick once this click bubbles up to the .dropdown li
}
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 const requestQuoteButtonNine = document.getElementById('quotenine');
 ...
 if (requestQuoteButtonNine) {
 requestQuoteButtonNine.addEventListener('click', function() {
 openModal(quoteFormModal);
 });
 }
 ...
 dropdownMenu.style.display = 'none';
 if (item1) item1.classList.remove('open');
 });
 ...
 if (serviceSelect.options[i].text === selectedService) {
 serviceSelect.selectedIndex = i;
 syncServiceDropdownDisplay(serviceSelect);
 break;
 }
 }
 }
 ...
 // Custom "Service" dropdown inside the booking/quote modals
 function syncServiceDropdownDisplay(selectEl) {
 const wrapper = selectEl.closest('.service-dropdown');
 if (!wrapper) return;
 const text = wrapper.querySelector('.service-dropdown-text');
 if (!text) return;
 ...
 text.textContent = 'Select a service';
 text.classList.add('placeholder');
 }
 }

 document.querySelectorAll('.service-dropdown').forEach(function(wrapper) {
 const trigger = wrapper.querySelector('.service-dropdown-trigger');
 const serviceMenu = wrapper.querySelector('.service-dropdown-menu');
 const nativeSelect = wrapper.querySelector('.service-dropdown-native');
 if (!trigger || !serviceMenu || !nativeSelect) return;

 const closeServiceMenu = function() {
 ...
 const isOpen = serviceMenu.style.display === 'block';
 document.querySelectorAll('.service-dropdown-menu').forEach(function(m) { m.style.display = 'none'; });
 document.querySelectorAll('.service-dropdown-trigger').forEach(function(t) { t.setAttribute('aria-expanded', 'false'); });
 if (!isOpen) {
 serviceMenu.style.display = 'block';
 trigger.setAttribute('aria-expanded', 'true');

 // On short/mobile viewports the modal's form area scrolls internally;
 // make sure the opened menu isn't clipped by that scroll container.
 const scrollParent = wrapper.closest('.form-content');
 if (scrollParent) {
 const menuRect = serviceMenu.getBoundingClientRect();
 const parentRect = scrollParent.getBoundingClientRect();
 const overflowAmount = menuRect.bottom - parentRect.bottom;
 if (overflowAmount > 0) {
 scrollParent.scrollTop += overflowAmount + 12;
 }
 }
 }
 });
 ...
 const item = event.target.closest('.service-dropdown-item');
 if (!item) return;
 event.stopPropagation();
 nativeSelect.value = item.getAttribute('data-value');
 nativeSelect.dispatchEvent(new Event('change', { bubbles: true }));
 syncServiceDropdownDisplay(nativeSelect);
 closeServiceMenu();
 });
...
function showSlide(index) {
 ...
 carouselViewport.scrollTo({ left: item.offsetLeft, behavior: 'smooth' });
}
...
if (carouselViewport && totalItems > 0) {
...
// Project Portfolio Lightbox
document.addEventListener('DOMContentLoaded', function () {
 const portfolioContainer = document.querySelector('.project-portfolio-container');
 const lightbox = document.getElementById('projectLightbox');
 if (!portfolioContainer || !lightbox) return;

 const lightboxImg = document.getElementById('projectLightboxImg');
 const closeBtn = document.getElementById('projectLightboxClose');
 const items = Array.from(portfolioContainer.querySelectorAll('.project-portfolio-item img'));
 let currentIndex = 0;
 ...
 const item = event.target.closest('.project-portfolio-item');
 if (!item) return;
 ...
 portfolioContainer.querySelectorAll('.project-portfolio-item').forEach(item => {
...
document.addEventListener('DOMContentLoaded', function() {
 try {
 emailjs.init("UQIAN58pk32SA6Sp5");
...
function sendMail(event, formId, responseMessageId) {
 event.preventDefault();
 ...
 name: form.querySelector('input[id="contactName"]')?.value || '',
 number: form.querySelector('input[id="contactContactNumber"]')?.value || '',
 email: form.querySelector('input[id="contactEmail"]')?.value || '',
 postcode: form.querySelector('input[id="contactPostCode"]')?.value || '',
 service: form.querySelector('select[id="contactService"]')?.value || '',
 details: form.querySelector('textarea[id="contactIssueDetails"]')?.value || ''
 };
 } else if (formId === 'bookingForm') {
 params = {
 formType: 'Booking Form',
 name: form.querySelector('input[id="bookingName"]')?.value || '',
 number: form.querySelector('input[id="bookingContactNumber"]')?.value || '',
 email: form.querySelector('input[id="bookingEmail"]')?.value || '',
 postcode: form.querySelector('input[id="bookingPostCode"]')?.value || '',
 service: form.querySelector('select[id="bookingService"]')?.value || '',
 details: form.querySelector('textarea[id="bookingIssueDetails"]')?.value || ''
 };
 } else if (formId === 'quoteForm') {
 params = {
 formType: 'Quote Form',
 name: form.querySelector('input[id="quoteName"]')?.value || '',
 number: form.querySelector('input[id="quoteContactNumber"]')?.value || '',
 email: form.querySelector('input[id="quoteEmail"]')?.value || '',
 postcode: form.querySelector('input[id="quotePostCode"]')?.value || '',
 service: form.querySelector('select[id="quoteService"]')?.value || '',
 details: form.querySelector('textarea[id="quoteIssueDetails"]')?.value || ''
 };
 } else {
 console.error("Form ID " + formId + " does not match any known forms.");
 return;
 }

 console.log("Form Data: ", params); // Debugging log

 emailjs.send("service_0a2ecad", "template_nkryzl2", params)
 .then(res => {
 // Clear form fields
 if (form instanceof HTMLFormElement) {
 form.reset();
 // Uncheck the checkbox explicitly if present
